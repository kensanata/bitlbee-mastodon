		...
		if (strcmp("content", k) == 0 && v->type == json_string && *v->u.string.ptr) {
			...
			for (i = 0; i < v->u.array.length; i++) {
				...
				if (mention->type == json_object) {
					...
					/* skip us as we only need these mentions for replies we write; also compare case-insensitively
					 * because mentions were generated by other users */
					if (acct && g_ascii_strcasecmp(acct, md->user) != 0) {
